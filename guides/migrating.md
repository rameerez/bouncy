...
3. Preserve manual holds independently. Map complaints as complaints and hard bounces as hard bounces. Account-list refusal is evidence of an existing restriction. Soft trackers are history; active legacy soft holds require an explicit preserved policy, not an accidental new threshold engine. If the old code escalated repeated soft bounces, import the active holds into `soft_blocked_until` **and** set `config.soft_bounce_threshold` and `config.soft_bounce_window` to the values it used, or the escalation silently stops at cutover while the imported holds expire.
